...
from keras.layers import Conv2D, MaxPooling2D, Conv1D, MaxPooling1D, Bidirectional, GRU
...
def create_model():
        model = Sequential()
        model.add(Bidirectional(GRU(n_hidden, return_sequences=True), input_shape=(zamanadimi, girdi_boyutu)))
        model.add(Bidirectional(GRU(np.int(n_hidden / 2), return_sequences=True), input_shape=(zamanadimi, girdi_boyutu)))
        model.add(Bidirectional(GRU(np.int(n_hidden / 4))))
...
model_name = 'bi_gru_ham3input-3.h5'
